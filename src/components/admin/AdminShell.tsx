...
import { ExternalLink, LayoutGrid, List, LogOut, Menu, MessageSquareText, Settings, Smartphone } from "lucide-react";
        ...
        <div className="min-w-0 flex-1">
          <header className="mb-4 rounded-3xl border border-border bg-card px-4 py-4 shadow-card sm:mb-6 sm:px-5">
            ...
            <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
              ...
              <SheetContent side="left" className="w-[min(86vw,20rem)] bg-sidebar p-4 text-sidebar-foreground sm:max-w-xs">
                <SheetTitle className="flex items-center gap-3 border-b border-sidebar-border pb-5 text-left text-sidebar-foreground">
                  <span className="grid size-10 place-items-center rounded-xl bg-sidebar-primary text-sidebar-primary-foreground"><Smartphone className="size-5" /></span>
                  <span><span className="block font-display text-lg font-bold">Warista</span><span className="block text-xs uppercase tracking-[0.18em] text-sidebar-foreground/60">Owner admin</span></span>
                </SheetTitle>
                <nav className="mt-6 space-y-1.5">
                  {navItems.map(({ to, label, icon: Icon, exact }) => (
                    <Link key={to} to={to} activeOptions={{ exact }} onClick={() => setMobileMenuOpen(false)} activeProps={{ className: "bg-sidebar-primary text-sidebar-primary-foreground shadow-sm" }} className="flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-semibold text-sidebar-foreground/70 transition-colors hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"><Icon className="size-4" /> {label}</Link>
                  ))}
                </nav>
                <div className="absolute inset-x-4 bottom-4 space-y-2 border-t border-sidebar-border pt-4">
                  <Button asChild variant="ghost" className="w-full justify-start gap-2 text-sidebar-foreground/70 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"><Link to="/" onClick={() => setMobileMenuOpen(false)}><ExternalLink className="size-4" /> View storefront</Link></Button>
